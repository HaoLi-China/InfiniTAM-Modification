...
MotionsData::MotionsData(ITMMotionAnalysis *motionAnalysis, float *depth, int depth_width, int depth_height)
{
	...
	std::cout << "no_nei_count:" << no_nei_count << std::endl;
		...
		for (int i = 0; i < points.size(); i++){
			if (visibles[i] != -1){
				...
				if (!((pt_image.x < 1) || (pt_image.x > depth_image_width - 2) || (pt_image.y < 1) || (pt_image.y > depth_image_height - 2))){
					...
					double dis = (dpt.x - vpt.x)*(dpt.x - vpt.x) + (dpt.y - vpt.y)*(dpt.y - vpt.y) + (dpt.z - vpt.z)*(dpt.z - vpt.z);
...
void ITMMotionAnalysis::optimizeEnergyFunctionNlopt(ITMFloatImage *newDepthImage)
{
	...
	//if (changeDpWhenIteration){
	//	for (int k = 0; k < 3; k++){
	//		double minf_tem;
	//		data.computeDpoints(depth, x, data.dpoints);
	//		result = opt.optimize(x, minf_tem);
	//		std::cout << "minf_tem = " << minf_tem << std::endl;
	//	}
	//}
